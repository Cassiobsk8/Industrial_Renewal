...
public class ItemCartLinker extends IRBaseItem {
    ...
    public static boolean thereIsNoConnectionLeft(AbstractMinecart cart) {
        return !hasFreeConnectionIn(cart, EnumCouplingType.COUPLING_1) && !hasFreeConnectionIn(cart, EnumCouplingType.COUPLING_2);
    }
    ...
    private static float getMaxCouplingDistance(AbstractMinecart cart1, AbstractMinecart cart2) {
        ...
        float dist = 0;
        if (cart1 instanceof ICoupleCart) dist += ((ICoupleCart) cart1).getMaxCouplingDistance(cart2);
        else dist += defaultDistance;
        if (cart2 instanceof ICoupleCart) dist += ((ICoupleCart) cart2).getMaxCouplingDistance(cart1);
        else dist += defaultDistance;
        return dist * dist;
    }
