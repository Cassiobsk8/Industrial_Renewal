...
public class BlockPillar extends BlockAbstractSixWayConnections {
    ...
    public boolean canConnectTo(Level worldIn, BlockPos currentPos, Direction neighborDirection) {
        ...
        if (neighborDirection != Direction.UP && neighborDirection != Direction.DOWN) {
            return nb instanceof LeverBlock || (nb instanceof BlockHVIsolator && neighborState.getValue(BlockHVIsolator.FACING) == neighborDirection.getOpposite()) || nb instanceof RedstoneTorchBlock || nb instanceof TripWireHookBlock || nb instanceof BlockColumn
                    ...
                    || nb instanceof LadderBlock || nb instanceof BlockElectricFence || nb instanceof BlockElectricGate || (nb instanceof BlockLight && neighborState.getValue(BlockLight.FACING) == neighborDirection.getOpposite())
                    //                    || nb instanceof BlockRoof
                    || (nb instanceof BlockBrace && Objects.equals(neighborState.getValue(BlockBrace.FACING).getName(), neighborDirection.getOpposite().getName())) || (nb instanceof BlockBrace && Objects.equals(neighborState.getValue(BlockBrace.FACING).getName(), "down_" + neighborDirection.getName()))
