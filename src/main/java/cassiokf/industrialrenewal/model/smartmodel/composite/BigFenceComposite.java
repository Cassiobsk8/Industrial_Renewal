...
public class BigFenceComposite extends BaseIBakedModel
{
    ...
    public List<BakedQuad> getQuads(@Nullable BlockState state, @Nullable Direction side, @Nonnull Random rand, @Nonnull IModelData extraData)
    {
        ...
        if (isLinkPresent(extraData, TEBigFenceBase.ACTIVE_LEFT))
        {
            quadsList.addAll(modelDown.getQuads(state, side, rand, extraData));
        }
        if (isLinkPresent(extraData, TEBigFenceBase.ACTIVE_RIGHT))
        {
            quadsList.addAll(modelUp.getQuads(state, side, rand, extraData));
        }
        if (isLinkPresent(extraData, TEBigFenceBase.ACTIVE_LEFT_DOWN))
        {
            quadsList.addAll(modelWest.getQuads(state, side, rand, extraData));
        }
        if (isLinkPresent(extraData, TEBigFenceBase.ACTIVE_RIGHT_DOWN))
        {
            quadsList.addAll(modelEast.getQuads(state, side, rand, extraData));
        }
        if (isLinkPresent(extraData, TEBigFenceBase.ACTIVE_LEFT_TOP))
        {
            quadsList.addAll(modelNorth.getQuads(state, side, rand, extraData));
        }
        if (isLinkPresent(extraData, TEBigFenceBase.ACTIVE_RIGHT_TOP))
        {
