...
public class IRConfig {
    ...
    public static final ForgeConfigSpec COMMOM;
    private static final ForgeConfigSpec.Builder BUILDER = new ForgeConfigSpec.Builder();

    static
    {
        IRMainConfig.init(BUILDER);

        COMMOM = BUILDER.build();
    }

    public static void loadConfig(ForgeConfigSpec config, String path)
    {
        IndustrialRenewal.LOGGER.info("Loading config: " + path);
        final CommentedFileConfig file = CommentedFileConfig.builder(new File(path)).sync().autosave().writingMode(WritingMode.REPLACE).build();
        IndustrialRenewal.LOGGER.info("Built config: " + path);
        file.load();
        IndustrialRenewal.LOGGER.info("Loaded config: " + path);
