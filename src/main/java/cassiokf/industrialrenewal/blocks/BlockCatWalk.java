...
public class BlockCatWalk extends BlockAbstractSixConnections
{
    ...
    protected boolean isValidConnection(final BlockState neighborState, final IBlockReader world, final BlockPos ownPos, final Direction neighborDirection)
    {
        ...
        if (neighborDirection != Direction.UP && neighborDirection != Direction.DOWN)
        {
            return nb instanceof BlockCatWalk
                    ...
                    || (nb instanceof BlockCatwalkLadder && neighborState.get(BlockCatwalkLadder.FACING) == neighborDirection && !(boolean)neighborState.get(BlockCatwalkLadder.ACTIVE));
        }
    ...
    protected VoxelShape getVoxelShape(BlockState state, IBlockReader worldIn, BlockPos pos, boolean collision)
    {
        ...
        {
            SHAPE = VoxelShapes.or(SHAPE, collision ? NORTH_AABB : RNORTH_AABB);
        }
        if (isConnected(state, Direction.SOUTH))
        {
            SHAPE = VoxelShapes.or(SHAPE, collision ? SOUTH_AABB : RSOUTH_AABB);
        }
        if (isConnected(state, Direction.WEST))
        {
            SHAPE = VoxelShapes.or(SHAPE, collision ? WEST_AABB : RWEST_AABB);
        }
        if (isConnected(state, Direction.EAST))
        {
            SHAPE = VoxelShapes.or(SHAPE, collision ? EAST_AABB : REAST_AABB);
        }
        return SHAPE;
    }
