...
public class BlockCatwalkStair extends BlockHorizontalFacing
{
    ...
    protected VoxelShape getVoxelShape(BlockState state, IBlockReader worldIn, BlockPos pos, boolean collision)
    {
        ...
        if (face == Direction.NORTH)
        {
            ...
            if (left)
            {
                FINAL_SHAPE = VoxelShapes.or(FINAL_SHAPE, collision ? WC_AABB : RWC_AABB);
            }
            if (right)
            {
                FINAL_SHAPE = VoxelShapes.or(FINAL_SHAPE, collision ? EC_AABB : REC_AABB);
            }
        ...
        if (face == Direction.WEST) {
            ...
            if (left) {
                FINAL_SHAPE = VoxelShapes.or(FINAL_SHAPE, collision ? SC_AABB : RSC_AABB);
            }
            if (right) {
                FINAL_SHAPE = VoxelShapes.or(FINAL_SHAPE, collision ? NC_AABB : RNC_AABB);
            }
        }
